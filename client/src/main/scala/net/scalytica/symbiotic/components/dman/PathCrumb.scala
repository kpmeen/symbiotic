...
object PathCrumb {
  ...
  class Backend($: BackendScope[Props, Props]) {
    ...
    def pathTag(pathSegment: Option[PathSegment], displayValue: ReactTag): ReactTag =
      <.li(<.a(^.cursor := "pointer", ^.onClick --> changePage(pathSegment.map(_.fid)))(displayValue))

    def pathTags(elems: Seq[(FileId, String)]): Seq[TagMod] = {
      var pb = Seq.newBuilder[String]
      val paths = elems.filterNot(_._2 == "root").map { e =>
        val fid = e._1
        val seg = e._2
        if (seg.nonEmpty) {
          pb += seg
      ...
      paths.zipWithIndex.map { path =>
        if (paths.size == CrumbLimit && path._2 == 0) pathTag(Option(path._1), <.span("..."))
