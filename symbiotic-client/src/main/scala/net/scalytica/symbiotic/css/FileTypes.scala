...
object FileTypes {
  ...
  object Styles extends StyleSheet.Inline {
    ...
    val fontIcon = styleF(ctDomain) {
      case Folder      => mixin(FontIcons.folder)
      case FolderOpen  => mixin(FontIcons.folderOpen)
      case PdfFile     => mixin(FontIcons.pdf)
      case TxtFile     => mixin(FontIcons.txt)
      case ArchiveFile => mixin(FontIcons.archive)
      case ImageFile   => mixin(FontIcons.image)
      case MovieFile   => mixin(FontIcons.movie)
      case SoundFile   => mixin(FontIcons.sound)
      case MSDocFile   => mixin(FontIcons.word)
      case MSXlsFile   => mixin(FontIcons.excel)
      case MSPptFile   => mixin(FontIcons.powerpoint)
      case _           => mixin(FontIcons.file)
    }

    val IconLg =
      styleF(ctDomain)(ct => styleS(fontIcon(ct), addClassName("fa-lg")))
    val Icon2x =
      styleF(ctDomain)(ct => styleS(fontIcon(ct), addClassName("fa-2x")))
    val Icon3x =
      styleF(ctDomain)(ct => styleS(fontIcon(ct), addClassName("fa-3x")))
    val Icon4x =
      styleF(ctDomain)(ct => styleS(fontIcon(ct), addClassName("fa-4x")))
    val Icon5x =
      styleF(ctDomain)(ct => styleS(fontIcon(ct), addClassName("fa-5x")))
