...
object UserProfilePage {
  ...
  case class Avatar(url: Option[String] = None, blob: Option[dom.Blob] = None) {
    ...
    private def blobToObjURL: Option[String] =
      blob.map(b => URL.createObjectURL(b))
  ...
  class Backend($ : BackendScope[Props, State]) {
    ...
    def loadContent(uid: String): Callback =
      Callback.future[Unit] {
        User.getUser(uid).flatMap {
            ...
            User.getAvatar(uid).map { ma =>
              $.modState(
                s =>
                  s.copy(
                    usr = Some(user),
                    avatar = Avatar(user.avatarUrl, ma),
                    status = Finished
                )
              )
            }
          case Left(failed) =>
            log.warn(
              s"Unable to retrieve user data for $uid. Reason: ${failed.msg}"
            )
            Future.successful(
              $.modState(
                s => s.copy(usr = None, avatar = Avatar(), status = failed)
              )
            )
        }
      }

    def showFileDialogue: Callback =
      Callback(jQuery("input[name=avatarFile]").click())
    ...
    def changeAvatar(e: ReactEventI): Callback = {
      ...
      $.state.map { s =>
        User.setAvatar(s.uid, form)(
          $.modState(s => s.copy(avatar = s.avatar.copy(blob = Option(file))))
        )
      }
    }
      ...
      <.div(
            ...
            state.status match {
              ...
              case Finished =>
                state.usr.fold(
                  <.div(
                    ^.className := "panel-body",
                    "Could not find any information for you."
                  )
                ) {
                  usr =>
                    <.div(
                      ...
                      <.div(
                        ...
                        <.div(
                          ...
                          <.form(
                            ...
                            <.img(
                              Style.avatar,
                              ^.src := state.avatar.calc(usr.useSocialAvatar),
                        ...
                        <.div(
                          ...
                          Option(usr.loginInfo.providerID)
                            .filter(_ == LoginInfo.credentialsProvider)
                            .map(
                              _ =>
                                <.div(
                                  <.label(
                                    Style.profileLabel,
                                    ^.`for` := s"profile_uname_${usr.id.get}",
                                    "Username:\u00a0"
                                  ),
                                  <.span(
                                    ^.name := s"profile_uname_${usr.id.get}",
                                    usr.username
                                  )
                              )
                            ),
                          <.div(
                            <.label(
                              Style.profileLabel,
                              ^.`for` := s"profile_email_${usr.id.get}",
                              "Email:\u00a0"
                            ),
                            <.span(
                              ^.name := s"profile_email_${usr.id.get}",
                              usr.emailOption
                            )
                          ),
                          <.div(
                            <.label(
                              Style.profileLabel,
                              ^.`for` := s"profile_dob_${usr.id.get}",
                              "Date of birth:\u00a0"
                            ),
                            <.span(
                              ^.name := s"profile_dob_${usr.id.get}",
                              usr.dateOfBirth
                                .map[String](d => new Date(d).toDateString())
                            )
                          ),
                          <.div(
                            <.label(
                              Style.profileLabel,
                              ^.`for` := s"profile_gender_${usr.id.get}",
                              "Gender:\u00a0"
                            ),
                            <.span(
                              ^.name := s"profile_gender_${usr.id.get}",
                              usr.readableGender.getOrElse[String]("")
                            )
                          )
                        )
                      )
        ...
        <.div(
          ...
          <.div(
            ...
            <.div(^.className := "panel-body", "column 2")
          )
        ),
        <.div(
          ^.className := "col-md-3",
          <.div(
            ...
            <.div(^.className := "panel-body", "column 3")
          )
        )
      )
    }
  ...
  val component = ReactComponentB[Props]("UserProfilePage")
    ...
    .componentWillReceiveProps(
      cwu => cwu.$.backend.loadContent(cwu.nextProps.uid)
    )
    .build
