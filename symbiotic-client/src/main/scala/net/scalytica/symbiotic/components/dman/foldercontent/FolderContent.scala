...
import net.scalytica.symbiotic.components.{
  FilterInput,
  IconButton,
  Modal,
  Spinner
...
object FolderContent {
  ...
  class Backend(val $ : BackendScope[Props, Props]) {
    ...
    def loadContent(p: Props) =
      p.selectedFolder.value
        .map(fid => ManagedFile.load(fid))
        .getOrElse(ManagedFile.load(None))
        .map {
          case Right(res) =>
            $.modState(
              _.copy(
    ...
    def uploadFile(e: ReactEventI): Callback =
      $.props.map { props =>
        ...
        ManagedFile.upload(props.selectedFolder.value, form)(
          Callback(loadContent().runNow())
        )
      }
    ...
    def downloadFile(e: ReactEventI): Callback =
      $.props.map(
        p =>
          p.selectedFile.value.foreach(
            f => dom.document.getElementById(f.fileId.value).domAsHtml.click()
        )
      )

    def createFolder[A <: ReactUIEvent](evt: A): Callback = {
      val fnameInput = Option(
        dom.document
          .getElementById(FolderNameInputId)
      ...
      fnameInput
        ...
        .map { in =>
          $.props.map { p =>
            Callback.future {
              ManagedFile
                .addFolder(p.selectedFolder.value, in.value)
                .map {
                  case Finished =>
                    $.state.flatMap(_.reloadFTree) >> loadContent()
     ...
     */
    def showIconView(e: ReactEventI): Callback =
      $.modState(s => s.copy(viewType = IconViewType))

    /**
     * Show the content as a Table view
     */
    def showTableView(e: ReactEventI): Callback =
      $.modState(s => s.copy(viewType = TableViewType))

    /**
     * Show the content as a Column view
     */
    def showColumnView(e: ReactEventI): Callback =
      $.modState(s => s.copy(viewType = ColumnViewType))
          ...
          p.selectedFile.value.map { mf =>
            ...
            val fmf: Future[Option[ManagedFile]] =
              if (mf.metadata.lock.isDefined) ManagedFile.unlock(fid).map {
                ...
                ManagedFile.lock(fid).map {
                  case Right(lock) =>
                    Some(
                      mf.copy(metadata = mf.metadata.copy(lock = Option(lock)))
                    )
                  case Left(err) =>
                    None
                }
        ...
        case Finished =>
          <.div(
            ...
            <.form(
              ...
              val pathElems = p.selectedFolder.value
                .map(fid => p.ftree.value.root.buildPathLink(fid))
              PathCrumb(
                p.selectedFolder,
                pathElems.getOrElse(Seq.empty),
                p.selectedFile,
                p.ctl
              )
            },
            <.div(
              ...
              <.div(
                ...
                p.selectedFile.value.map { mf =>
                  val lockCls =
                    mf.metadata.lock.fold("fa fa-lock")(l => "fa fa-unlock")
              ...
              <.div(
                ...
                FilterInput(
                  id =
                    s"searchBox-${p.selectedFolder.value.map(_.value).getOrElse("NA")}",
              ...
              body = {
                <.div(
                  ...
                  ^.onKeyPress ==> { (e: ReactKeyboardEvent) =>
                    if (e.key == "Enter") createFolder(e) else Callback.empty
                  },
  ...
  val component = ReactComponentB[Props]("FolderContent")
    ...
    .componentDidMount(
      $ => Callback.when($.isMounted())($.backend.loadContent())
    )
