...
class TaskProcessor(tid: TaskId) extends PersistentActor with TaskOperations with ActorLogging {
  ...
  override def receiveCommand: Receive = {
    ...
    case MoveTask(by, stepId, tstate, due) => journalAndPublish(TaskMoved(by, stepId, tstate, due))
    case ApproveTask(by, due) => journalAndPublish(TaskApproved(by, due))
    case RejectTask(by, due) => journalAndPublish(TaskRejected(by, due))
    case DeclineTask(by, due) => journalAndPublish(TaskDeclined(by, due))
    case ConsolidateTask(by, due) => journalAndPublish(TaskConsolidated(by, due))
