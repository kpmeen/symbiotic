...
object FolderPath extends DManFS {

  val logger = LoggerFactory.getLogger(FolderPath.getClass)

  implicit val reads: Reads[FolderPath] = __.readNullable[String].map(fromDisplay)
  implicit val writes: Writes[FolderPath] = Writes {
    case f: FolderPath => JsString(FolderPath.toDisplay(f))
  }

  val rootFolder: FolderPath = FolderPath("root")
   ...
   * @return Folder
   */
  def fromDBObject(dbo: DBObject): FolderPath = {
    dbo.getAs[DBObject](MetadataKey).flatMap(dbo =>
      dbo.getAs[String](PathKey.key).map(FolderPath.apply)
    ).getOrElse(rootFolder)
  }

  def regex(p: FolderPath, subFoldersOnly: Boolean = false): Regex = {
    val base = s"^${p.materialize}"
    if (subFoldersOnly) (base + "[^,\r\n]*,$").r
    else base.r
  }

  private def toDisplay(p: FolderPath): String = Option(p.path).getOrElse("/")

  private def fromDisplay(s: Option[String]): FolderPath = s.map(FolderPath.apply).getOrElse(rootFolder)
   ...
   */
  def exists(cid: CustomerId, at: FolderPath): Boolean =
   ...
   */
  def save(cid: CustomerId, at: FolderPath = FolderPath.rootFolder): Option[FolderId] = {
    if (!exists(cid, at)) {
   ...
   */
  def move(cid: CustomerId, orig: FolderPath, mod: FolderPath): CommandStatus[Int] = {
   ...
   */
  def filterMissing(cid: CustomerId, f: FolderPath): List[FolderPath] = {

    case class CurrPathMiss(path: String, missing: List[FolderPath])
   ...
   */
  def treeNoFiles(cid: CustomerId, from: FolderPath = FolderPath.rootFolder): Seq[FolderPath] = {
    ...
    tree[Option[FolderPath]](cid, query, fields)(mdbo =>
      mdbo.getAs[DBObject](MetadataKey).flatMap(dbo => dbo.getAs[String](PathKey.key).map(FolderPath.apply))
   ...
   */
  def treeWith[A](cid: CustomerId, from: FolderPath = FolderPath.rootFolder)(f: (MongoDBObject) => A): Seq[A] = {
   ...
   * @return a collection of A instances
   */
  def childrenWith[A](cid: CustomerId, from: FolderPath = FolderPath.rootFolder)(f: (MongoDBObject) => A): Seq[A] = {
    tree(cid, $and (
