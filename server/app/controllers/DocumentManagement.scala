...
class DocumentManagement @Inject()(
  ...
  def getFolderHierarchy(path: Option[String]) = SecuredAction {
    implicit request =>
      ...
      val folders: Seq[(FileId, Path)] =
        dmService.treePaths(path.map(Path.apply))(uid)
  ...
  def addFolderToPath(fullPath: String, createMissing: Boolean = true) =
    SecuredAction { implicit request =>
      ...
      dmService
        .createFolder(Path(fullPath), createMissing)(currUsrId)
  ...
  def addFolderToParent(parentId: String, name: String) =
    SecuredAction { implicit request =>
      val currUsrId = request.identity.id.get
      dmService
        .getFolder(FileId.asId(parentId))(currUsrId)
        .map { f =>
          val currPath = f.flattenPath
          dmService
            .createFolder(currPath.copy(s"${currPath.path}/$name"))(currUsrId)
            .map(fid => Created(Json.toJson(fid)))
            .getOrElse(
              BadRequest(
                Json.obj(
  ...
  def changeFolderName(orig: String, mod: String) = SecuredAction {
    implicit request =>
      val currUsrId = request.identity.id.get
  ...
  def moveFolderTo(orig: String, mod: String) = SecuredAction {
    implicit request =>
      val currUsrId = request.identity.id.get
  ...
  def moveFileTo(fileId: String, orig: String, dest: String) =
    SecuredAction { implicit request =>
      ...
      dmService
        .moveFile(fileId, Path(orig), Path(dest))(currUsrId)
