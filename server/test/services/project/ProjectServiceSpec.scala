...
class ProjectServiceSpec extends Specification with MongoSpec {
  ...
  "When using the ProjectService it" should {
    ...
    "be possible to update a Project" in {
      ...
      val prj2 = ProjectService.findById(prj1.id.get)
      prj2 must_!= None

      val mod = prj2.get.copy(description = Some("Foo description"))

      ProjectService.save(mod) must_== Updated

      val prj3 = ProjectService.findById(prj1.id.get)
      prj3 must_!= None
      prj3.get.description must_== mod.description
    }
  }
