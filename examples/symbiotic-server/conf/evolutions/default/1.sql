...
CREATE INDEX files_file_id_index
  ON symbiotic_dman.files (file_id);
CREATE INDEX files_file_name_index
  ON symbiotic_dman.files (file_name);
CREATE INDEX files_path_index
  ON symbiotic_dman.files (path);
CREATE INDEX files_is_folder_index
  ON symbiotic_dman.files (is_folder);
CREATE INDEX files_owner_id_index
  ON symbiotic_dman.files (owner_id);

-- ====================================================================
-- Support tables for general application functionality
-- ====================================================================
CREATE TABLE symbiotic_dman.avatars (
  id           UUID PRIMARY KEY DEFAULT uuid_generate_v1mc(),
...
CREATE INDEX avatar_user_id_index
  ON symbiotic_dman.avatars (user_id);
CREATE INDEX avatar_filename_index
  ON symbiotic_dman.avatars (filename);

CREATE TABLE symbiotic_dman.users (
  id                UUID PRIMARY KEY DEFAULT uuid_generate_v1mc(),
...
CREATE INDEX users_username_index
  ON symbiotic_dman.users (username);
CREATE INDEX users_provider_index
  ON symbiotic_dman.users (provider_id, provider_key);


CREATE TABLE symbiotic_dman.password_info (
...
CREATE INDEX pinfo_provider_index
  ON symbiotic_dman.password_info (provider_id, provider_key);

CREATE TABLE symbiotic_dman.oauth2info (
  id            SERIAL PRIMARY KEY NOT NULL,
  provider_id   TEXT               NOT NULL,
  provider_key  TEXT               NOT NULL,
...
CREATE INDEX oauth2_access_token
  ON symbiotic_dman.oauth2info (access_token);


# --- !Downs

DROP SCHEMA symbiotic_dman CASCADE;